...
function parseEffectiveFrom(effectiveFrom: string | Date | null | undefined) {
...
export function getPayableDaysInMonth({
  effectiveFrom,
  year,
  month,
}: {
  ...
  const start = parseEffectiveFrom(effectiveFrom ?? null);
...
export function calculateMonthlyPayout({
  ...
  const datesInMonth = leaveDatesInMonth({
    leaveDates,
    year,
    month,
  ...
  if (!notStarted && payableDays > 0) {
    baseSalary = isPartialMonth ? Math.round(dailyRate * payableDays) : salary;
  }
